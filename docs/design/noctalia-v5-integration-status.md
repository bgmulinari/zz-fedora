...
- Beta4 raises the current plugin API to 8 while explicitly keeping APIs 3 through 8 compatible. The managed `local/niri-outputs` plugin remains at API 3 because it uses none of the new API 8 surfaces; beta4 loads it as enabled, and offline lint reports zero errors and zero warnings.
- The release adds encrypted persistence backed by libsodium and libsecret. Fedora's package has direct runtime requirements for both libraries. A Secret Service provider remains a separate runtime recommendation, so `gnome-keyring` moved from the minimal-desktop-skippable GTK portal unit into `base-noctalia`; every supported profile now has durable storage for Noctalia credentials, encrypted clipboard history, and calendar cache. The live host exposes `org.freedesktop.secrets` through GNOME Keyring, D-Bus activated: the `gnome-keyring-daemon` socket unit is disabled and its three autostart units skip on their exec conditions.
- `gnome-keyring-pam` is installed alongside it. Fedora splits `pam_gnome_keyring.so` into that subpackage, and greetd already ships `-auth optional pam_gnome_keyring.so` plus `-session optional pam_gnome_keyring.so auto_start` in `/etc/pam.d/greetd`; the leading `-` silently skips both lines while the module is absent. Without it the login keyring is never unlocked from the login password, so a fresh install reaches `StorageKeyState::DeniedOrLocked` and clipboard history stops persisting with no error at the point of use. The hook is greetd-specific: `/etc/pam.d/login` carries no keyring line, so a TTY login into the session does not unlock the keyring.
- Beta4 also replaces the external Python Pywalfox host with Noctalia's built-in, multi-profile Firefox native messaging host. The Firefox bundle now uses the generic `browsers-firefox-theme` / `firefox-theme` integration, calls `noctalia firefox-theme install`, and retains only the user-disableable signed Firefox extension policy. It no longer installs `pipx`, the Python native host, or the ZZ-managed socket-watching path and service units.
- The managed community template selection changed from the legacy `pywalfox` template to `pywalfox-beta4`. That template writes the same `colors.json` but uses `post_action = "firefox-theme"`, so Noctalia applies palette and dark/light changes itself. A fresh isolated `HOME` test confirmed beta4 writes a valid native-messaging manifest pointing to `/usr/bin/noctalia`.
- No existing-install migration was added, but the native-messaging manifest converges on its own. The `firefox-theme` verifier applies Noctalia's own ownership test—a host program named `noctalia` or `noctalia-pywalfox`, still installed—so a manifest naming any other host is not converged and the action reinstalls it. Noctalia's `firefox-theme install` rewrites the manifest unconditionally, unlike the template `post_action`, which leaves a host it does not own in place. The development host's manifest therefore still points to `~/.local/bin/pywalfox` only until the next install run. What does persist as local legacy state is the pipx-installed Python host and the former synchronization path unit, which remains enabled; removing those installed artifacts is an explicit host operation, separate from the clean product baseline. That unit is harmless in the meantime because beta4 binds the same `/tmp/pywalfox_socket_<uid>` and republishes legacy socket traffic to its secondary profile hosts.
- The verifier deliberately does not pin the manifest to one resolved path. Noctalia rewrites the manifest from whichever binary is running, so requiring byte equality with the installer's `command -v noctalia` would make a locally built or otherwise differently located shell fight the action on every run.
- The wider beta4 feature set—new bar widgets and taskbar modes, panel anchoring, encrypted clipboard controls, CalDAV credentials, OSD additions, and plugin API 8 features—does not require new product defaults. The existing managed config validates unchanged apart from the Firefox template selection.
- No Settings UI state was promoted. The host currently keeps Ayu as a local theme override and `AutumnCanopy.jpg` as its local wallpaper; favorite/last/per-output wallpaper data and widget/output geometry remain app-managed or hardware-specific state.
- Catalog validation, the focused action/planner/catalog/post-action/first-run/config/Niri Outputs checks, the 97-test smoke gate, the 356-test full suite, and a full non-applying installer dry run pass against beta4.
...
- Beta3 makes plugin API 3 metadata mandatory. The managed `local/niri-outputs` plugin already declares `plugin_api = 3`; the current shell reports no plugin lint errors or warnings, and its focused Bats suite passes.
- Fedora 44's standard repositories still lag upstream: stable has beta1 and Updates Testing has beta2. The beta2 plugin loader expects the older `min_noctalia` field and rejects the managed API 3 plugin, so the official package is not the current integration target.
- `base-noctalia` is now a normal DNF bundle containing unpinned `noctalia-git` from `copr:lionheartp/Hyprland`. Installer reruns use the regular DNF transaction, so the package follows the newest build available from that COPR without a beta-specific action, minimum-version check, or `updates-testing` override.
- Terra excludes both `noctalia-git` and `noctalia-greeter`, keeping the shell and greeter on the intended COPR while Terra remains enabled for Ghostty.
- Keep this rolling COPR package until upstream v5 stable is available as Fedora's standard `noctalia` package. At that point, switch the manifest back to `noctalia` and remove the shell-specific Terra exclusion.

## 2026-07-12 native Niri Outputs plugin

- Added the local `local/niri-outputs` Noctalia v5 plugin under the ZZ-managed `noctalia` config tree. It provides a lightweight bar launcher, optional Control Center shortcut, and fixed-size panel that attaches to the bar and opens near its launcher by default. The panel queries Niri when opened, refreshed, reset, or reconciled after keep or restore; no plugin process polls in the background.
...
- `base-login-manager` now runs the required `noctalia-greeter` action instead of installing `sddm`. The action installs/syncs the COPR greeter package, writes `/etc/greetd/config.toml` for `/usr/bin/noctalia-greeter-session`, prepares the `greeter` account and `/var/lib/noctalia-greeter`, labels that state as `xdm_var_lib_t` for SELinux-confined greeter writes, patches `/etc/pam.d/greetd` for `pam_systemd.so` or `pam_elogind.so` when available, initializes `greeter.toml`, and enables `greetd.service`.
- The existing display-manager policy is preserved: if any display manager is already enabled, including SDDM, GDM, Plasma Login Manager, LightDM, Ly, or greetd, the Noctalia Greeter fallback action records a skip and does not install or enable another login manager. When the enabled greetd instance uses the managed Noctalia Greeter config and state, the action first reconciles and verifies the state directory's SELinux context so installer reruns repair affected systems without reinstalling the greeter.
...
- The official Fedora beta4 build runs on the current development host with the managed config, loads the `local/niri-outputs` plugin, and responds over IPC. Config validation and plugin lint report no errors or warnings.
...
- Built-in templates enabled by the managed config: `niri`, `starship`, `btop`, `gtk3`, `gtk4`, and `kcolorscheme`.
- Community templates enabled by the managed config: `pywalfox-beta4`, `zen-browser`, `neovim`, `vscode`, `zed`, and `yazi`. No v4 plugins, QuickShell config, or migration shims are present.
