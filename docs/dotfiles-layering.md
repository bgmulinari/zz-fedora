...
| Nautilus | none; the extension link is product-owned (user extensions go beside it in `~/.local/share/nautilus-python/extensions/`) | `dotfiles/nautilus/.local/share/nautilus-python/extensions/zz-copy-path.py`, a nautilus-python extension that adds Copy Full Path to the file, folder, and background context menus |
...
Niri window rules follow the same model in
`~/.config/niri/dms/windowrules.kdl`. Settings → Window Rules lists the
rules it reaches by following `include` lines from `config.kdl` literally,
and it never expands the `~/.zz/...` include of the product defaults, so a
`window-rule` in the product `cfg/` tree is applied by niri but invisible to
the UI. Each seeded rule carries the `// @id=... @name=...` line DMS
round-trips; the `dms-floating-windows` id is DMS's own, so its Floating
Windows toggle finds the seeded rule instead of adding a duplicate. Layer
rules without DMS controls stay in the product tree, and the
default corner-radius rule is generated by DMS into `dms/layout.kdl`.

`templates/niri/dms-layout.kdl` seeds only the Xray preferences, with Xray
and Dank Bar Xray off. DMS reads `xray false` and the `// bar-xray off`
marker from this file before regenerating its layout; these preferences are
not JSON settings. The file is seeded once, then owned by DMS. Its generated
geometry and other layout defaults are not copied into the seed.

The DMS settings seed enables compact notifications, the timeout progress
bar, and running the default action when clicking a notification body.
Global, popout, modal, and notification animations inherit DMS's Short
default, with component animation synchronization also inheriting the enabled default.

The launcher button uses Fedora's packaged SVG at
`/usr/share/icons/hicolor/scalable/apps/start-here.svg`, with the Surface color
override and a -2% size offset. The GTK appearance base unit installs
`fedora-logos` so this system-wide asset is available on fresh installs.

The System Update bar widget sits between Notification Center and Control
Center. DMS checks for updates at startup and once a day; its update action
runs `zz update all` in the automatically selected terminal. The widget hides
when no updates are available; Flatpak inclusion inherits the DMS default.
