...
module_05_bootstrap_tools() {
  ...
  if [[ "${ZZ_INSTALLER_APPLY_RELEASE_UPDATES:-0}" -eq 1 ]]; then
    if declare -F distro_apply_release_updates >/dev/null 2>&1; then
      distro_apply_release_updates || return 1
    else
      die "Release updates were requested, but the distro adapter does not implement them."
    fi
  fi
}
