...
@test "print-plan JSON emits machine-readable plan without log prefix" {
  ...
  assert_contains "$output" '"dms-greeter"'
  assert_contains "$output" '"dms"'
  assert_contains "$output" '"user_wants":[{"unit":"niri.service","wants":"dms.service"}]'
  assert_contains "$output" '"copr:avengemedia/dms"'
  assert_contains "$output" '"copr:avengemedia/danklinux"'
  assert_contains "$output" '"browsers-firefox"'
  assert_contains "$output" '"python3-pip"'
