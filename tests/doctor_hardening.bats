...
@test "base responsibility and managed config policy include critical rationale rows" {
  ...
  assert_tsv_row "$ROOT_DIR/config/base-responsibility.tsv" $'action\tnoctalia-greeter\tdesktop-service\tgraphical login\tInstalls Noctalia Greeter from COPR, configures greetd, seeds the managed theme and wallpaper as the greeter appearance, and enables the fallback graphical login.'
