"""Installation task for the ZZ Linux Setup Anaconda add-on."""
...
from org_zz_linux_setup.constants import SELECTION_FILE

log = logging.getLogger(__name__)

SOURCE_REPO_DIR = Path("/run/install/repo/zz-linux-setup")
TARGET_SELECTION_PATH = Path("root/zz-linux-setup-install-selected")
TASK_LOG_PATH = Path("root/zz-linux-setup-kickstart.log")
RUN_SCRIPT_PATH = Path("root/zz-linux-setup-run-install.sh")
...
class ZZLinuxSetupInstallationTask(Task):
    ...
    @property
    def name(self):
        return "Install ZZ Linux Setup"

    def run(self):
        """Copy the repo into the target system and run the installer."""

        self._report("Preparing ZZ Linux Setup")
        ...
        state_dir = target_home / ".local/state/zz-linux-setup"
        cache_dir = target_home / ".cache/zz-linux-setup"
        config_dir = target_home / ".config/zz-linux-setup"
        ...
        self._report("Starting ZZ Linux Setup for {}".format(user["name"]))
        self._run_installer(progress_path)
        self._cleanup_runtime_files()
        self._report("ZZ Linux Setup complete")
    ...
    def _copy_repo(self, target_repo_path, uid, gid):
        self._report("Copying ZZ Linux Setup into the target system")
    ...
    def _write_selection_config(self, selection_lines, destination, user):
        ...
        with open(destination, "w", encoding="utf-8") as state_file:
            state_file.write("distro=fedora\n")
...
export ZZ_INSTALLER_APPLY_RELEASE_UPDATES=1
...
""".format(
    ...
    def _run_installer(self, progress_path):
        ...
        with open(task_log_path, "w", encoding="utf-8") as task_log:
            task_log.write("[zz-linux-setup] Starting Anaconda task\n")
            task_log.flush()
            ...
            if process.returncode != 0:
                self._report("ZZ Linux Setup failed")
                raise RuntimeError(
                    "ZZ Linux Setup failed with exit code {}. Check {} and "
                    "the target user's zz-linux-setup logs.".format(
                        process.returncode,
                        "/" + str(TASK_LOG_PATH),
                    )
                )
    ...
    def _consume_progress(self, progress_path, task_log):
        ...
        for raw_line in lines:
            ...
            if label and label != "ZZ Linux Setup":
            ...
            task_log.write("[zz-linux-setup-progress] {}\n".format(message))
            task_log.flush()
    ...
    def _current_step_prefix(self):
        if self._current_step_label:
            if self._current_step_current and self._current_step_total:
                return "ZZ Linux Setup ({}/{}): {}".format(
                    self._current_step_current,
                    self._current_step_total,
                    self._current_step_label,
                )
            return "ZZ Linux Setup: {}".format(self._current_step_label)
        return "ZZ Linux Setup"
    ...
    def _format_progress(self, status, current, total, label, detail):
        if label == "ZZ Linux Setup" and status == "done":
            return "ZZ Linux Setup complete"
        if label == "ZZ Linux Setup" and status == "failed":
            return "ZZ Linux Setup failed"
        ...
        if status == "running" and detail:
            return "ZZ Linux Setup{}: {} - {}".format(step_suffix, label, detail)

        return "ZZ Linux Setup{}: {} {}".format(step_suffix, verb, label)
